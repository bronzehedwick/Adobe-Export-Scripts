...
function processLayer(layer)
{
	...
	check = 0;
	for (var ii = 0; ii < layerName.length; ii++)  // loop through layer name that was split into an array
	{
			// Gather Name
...
{ 
	...
	else 
	{
		...
		for (var ii = 0; ii < childName.length; ii++)  // loop through child name that was split into an array to pick out tags
		{
			// Gather name
		...
		{
	//		if (child.file.exists === true)
		//		nameO[i] = unescape(child.file.name);
//			else
...
//~ 			if (kerningMethod[i] != undefined)
//~ 				kerningMethod[i] = child.textRange.characterAttributes.kerningMethod;
...
//~ 				tab("\"kerning method\":\"" + kerningMethod[i] + "\"" + ",\n");
